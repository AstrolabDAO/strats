 ...
 *
 * @title AlgebraProvider - Algebra DEX adapter to retrieve TWAPs
 * @author Astrolab DAO
 * @notice Retrieves, validates and converts any of Algebra pool prices (https://algebra.finance)
 */
contract AlgebraProvider is PriceProvider {
  ...
  function _poolSqrtTwapX96(
    ...
    if (_period == 0) {
      ...
      (int56[] memory tickCumulatives, , , ) = _pool.getTimepoints(lookback);
   ...
   * @param _pool Algebra pool containing `_base` and `_quote`
   * @param _base Base token
   * @return price Price of `_base` in `_quote` wei
   */
  function _poolPrice(
    IAlgebraPool _pool,
    address _base
  ...
  /**
   * @notice Updates the Algebra feeds
   * @param _params Encoded Algebra specific parameters
   */
  ...
  function _setFeed(
    ...
    IAlgebraPool pool = IAlgebraPool(_feed.toAddress());
    (address base, address quote) = (pool.token0(), pool.token1());
    PoolDesc memory desc = PoolDesc({
      pool: pool,
      base: base,
      quote: quote
    });
    ...
    // descByPoolId[poolId(quote, base)] = desc; // not needed since resolvePoolId is symmetric (but can save runtime gas)
  }
}
