...
interface IAs4626 is IAs4626Abstract {
  function init(Fees memory _fees, address _asset, address _feeCollector) external;
  ...
  function setMinLiquidity(uint256 minLiquidity) external;
  function setProfitCooldown(uint256 profitCooldown) external;
  function setRedemptionRequestLocktime(uint256 _redemptionLocktime) external;
