...
interface IQiToken is IQiTokenStorage {
  ...
  event NewReserveFaqitor(
    uint256 oldReserveFaqitorMantissa, uint256 newReserveFaqitorMantissa
  );
  event ReservesAdded(address benefaqitor, uint256 addAmount, uint256 newTotalReserves);
  ...
  function _setReserveFaqitor(uint256 newReserveFaqitorMantissa)
    external
    returns (uint256);
