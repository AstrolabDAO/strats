...
contract BenqiMultiStake is StrategyV5Chainlink {
  ...
  IQiToken[8] internal _qiTokens; // LP token/pool
  ...
  function setParams(Params calldata _params) public onlyAdmin {
    ...
    for (uint8 i = 0; i < _params.qiTokens.length; i++) {
      _qiTokens[i] = IQiToken(_params.qiTokens[i]);
      _qiTokenDecimals[i] = _qiTokens[i].decimals();
    }
  ...
  function _invest(
    ...
    for (uint8 i = 0; i < _inputLength; i++) {
      ...
      uint256 iouBefore = _qiTokens[i].balanceOf(address(this));
      _qiTokens[i].mint(toDeposit);

      uint256 supplied = _qiTokens[i].balanceOf(address(this)) - iouBefore;
  ...
  function _setAllowances(uint256 _amount) internal override {
    for (uint8 i = 0; i < _inputLength; i++) {
      inputs[i].forceApprove(address(_qiTokens[i]), _amount);
    }
  }
  ...
  function _stakedInput(uint256 _index) internal view override returns (uint256) {
    return _stakeToInput(_qiTokens[_index].balanceOf(address(this)), _index);
  }
