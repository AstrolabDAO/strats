...
interface ISingleStaking {
  ...
  function pendingTokens(
    ...
    external
    view
    returns (uint256 pendingesVKA, address bonusTokenAddress, uint256 pendingBonusToken);

  function massUpdatePools() external;

  function updatePool(uint256 _pid) external returns (PoolInfo memory pool);

  function unstakeAndLiquidate(uint256 _pid, address _user, uint256 _amount) external;
  ...
  function rewarderBonusTokenInfo(uint256 _pid)
    external
    view
    returns (address bonusTokenAddress);
