 ...
 *
 * @title LodestarMultiStake Strategy - Liquidity providing on Lodestar
 * @author Astrolab DAO
...
contract LodestarMultiStake is StrategyV5Chainlink {
  ...
  ILToken[] internal _lTokens; // LP token/pool
  uint8[8] internal _lTokenDecimals; // Decimals of the LP tokens
  ...
  function setParams(Params calldata _params) public onlyAdmin {
    ...
    _lTokens = new ILToken[](_params.lTokens.length);
    for (uint8 i = 0; i < _params.lTokens.length; i++) {
      _lTokens[i] = ILToken(_params.lTokens[i]);
      _lTokenDecimals[i] = _lTokens[i].decimals();
    }
   ...
   * @param _lodestarParams Lodestar specific parameters
   */
  function init(
  ...
  function _invest(
    ...
    for (uint8 i = 0; i < _inputLength; i++) {
      ...
      uint256 iouBefore = _lTokens[i].balanceOf(address(this));
      _lTokens[i].mint(toDeposit);

      uint256 supplied = _lTokens[i].balanceOf(address(this)) - iouBefore;
  ...
  function _setAllowances(uint256 _amount) internal override {
    for (uint8 i = 0; i < _inputLength; i++) {
      inputs[i].forceApprove(address(_lTokens[i]), _amount);
    }
  }
  ...
  function _stakeToInput(
    ...
    return _amount.mulDiv(_lTokens[_index].exchangeRateStored(), 1e18);
  }
  ...
  function _inputToStake(
    ...
    return _amount.mulDiv(1e18, _lTokens[_index].exchangeRateStored());
  }

  /**
   * @notice Returns the invested input converted from the staked LP tokene
   * @return Input value of the LP/staked balance
   */
  function _stakedInput(uint256 _index) internal view override returns (uint256) {
    return _stakeToInput(_lTokens[_index].balanceOf(address(this)), _index);
  }
