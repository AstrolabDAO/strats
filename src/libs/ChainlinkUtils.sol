...
library ChainlinkUtils {
    ...
    /**
     * @notice Computes the input/underlying exchange rate from Chainlink oracle price feeds in _baseDecimals
     * @dev Used by invested() to compute input->underlying (base/quote, eg. USDC/BTC not BTC/USDC)
     * @param _priceFeeds Chainlink oracle price feeds [quote,base] eg. [input,underlying]
     * @return The amount available for investment
     */
    function underlyingExchangeRate(IChainlinkAggregatorV3[2] calldata _priceFeeds, uint8 _baseDecimals, uint8 _baseFeedDecimals) public view returns (uint256) {

        if (address(_priceFeeds[0]) == address(_priceFeeds[1]))
            return 10 ** uint256(_baseDecimals); // == weiPerUnit of underlying == 1:1
