...
contract FeesTest is TestEnvArb {
  ...
  function entryFees(Fees memory _fees, uint256 _minLiquidity) public {
    ...
    require(depositCollectedFee == depositClaimableFee, "Deposit entry fees collected does not match claimable");
    require(depositCollectedFee + strat.assetsOf(bob) == deposited, "Deposit entry fee accounting error");
    require(depositCollectedFee == strat.assetsOf(manager), "Collected fees vs feeCollector assets mismatch");
    ...
    require(mintCollectedFee == mintClaimableFee, "Mint entry fee collected does not match claimable");
    require(mintCollectedFee + strat.assetsOf(bob) == deposited, "Mint entry fee accounting error");
    require(mintCollectedFee == strat.assetsOf(manager), "Collected fees vs feeCollector assets mismatch");
  ...
  function exitFees(Fees memory _fees, uint256 _minLiquidity) public returns (uint256) {
    ...
    require(withdrawCollectedFee == withdrawClaimableFee, "Withdrawal exit collected does not match claimable");
    require(strat.assetsOf(bob) == toDeposit - withdrawn - withdrawCollectedFee, "Withdraw exit fee accounting error");
    require(withdrawCollectedFee == strat.assetsOf(manager), "Collected fees vs feeCollector assets mismatch");
    ...
    require(redeemCollectedFee == redeemClaimableFee, "Redeem exit collected does not match claimable");
    require(strat.assetsOf(bob) == toDeposit - withdrawn - withdrawCollectedFee, "Redeem exit fee accounting error");
    require(redeemCollectedFee == strat.assetsOf(manager), "Collected fees vs feeCollector assets mismatch");
    require(withdrawCollectedFee == redeemCollectedFee, "Withdraw and redeem fees do not match");
  ...
  function managementFees(Fees memory _fees, uint256 _minLiquidity) public {
    ...
    uint256 theoreticalMgmtFeeShares = strat.totalSupply().mulDiv(strat.fees().mgmt * duration, AsMaths.BP_BASIS * AsMaths.SEC_PER_YEAR);
    vm.prank(manager);
    uint256 mgmtFeeShares = strat.collectFees();
    require(mgmtFeeShares == theoreticalMgmtFeeShares, "Management fee does not match (shares)");
  ...
  function performanceFees(Fees memory _fees, uint256 _minLiquidity) public {
    ...
    uint256 profitBps = (sharePriceAfter - sharePriceBefore).mulDiv(AsMaths.BP_BASIS, sharePriceAfter); // eg. 50% of current share price
    uint256 theoreticalPerfFee = (profitBps * strat.totalAssets()).mulDiv(strat.fees().perf, AsMaths.BP_BASIS ** 2);
    uint256 theoreticalPerfFeeShares = strat.convertToShares(theoreticalPerfFee);

    assetsBefore = strat.totalAssets();
    vm.prank(manager);
    uint256 perfFeeShares = strat.collectFees();
    require(perfFeeShares == theoreticalPerfFeeShares, "Performance fee does not match (shares)");
    checkSumOfAssets(assetsBefore); // dynamic fees are inflationary, minted shared slightly dilute total assets
  }
  ...
  function flashLoanFees(Fees memory _fees, uint256 _minLiquidity) public {
    ...
    require(effectiveFlashFee == strat.flashFee(USDC, address(borrower), toBorrow), "Flash fee mismatch");
    ...
    require(strat.balanceOf(manager) == theoreticalFlashFeeShares, "Flash fee accounting error");
  ...
  function checkSumOfAssets(uint256 _expected) public {
    ...
    uint256 sumOfAssets = strat.assetsOf(bob) + strat.assetsOf(manager) + strat.assetsOf(admin);
    require(assets <= (_expected + 1) && assets >= (_expected - 1), "Total assets inflated or deflated");
    require(strat.totalAssets() <= (sumOfAssets + 3) && strat.totalAssets() >= (sumOfAssets - 3), "Total assets do not add up"); // 3 = 1wei max flooring error per user
  }

  function flow(Fees memory _fees, uint256 _minLiquidity) public {
    deployDependencies();
    entryFees(Fees({perf: 0, mgmt: 0, entry: _fees.entry, exit: 0, flash: 0}), _minLiquidity);
    exitFees(Fees({perf: 0, mgmt: 0, entry: 0, exit: _fees.exit, flash: 0}), _minLiquidity);
    managementFees(Fees({perf: 0, mgmt: _fees.mgmt, entry: 0, exit: 0, flash: 0}), _minLiquidity);
    performanceFees(Fees({perf: _fees.perf, mgmt: 0, entry: 0, exit: 0, flash: 0}), _minLiquidity);
    flashLoanFees(Fees({perf: 0, mgmt: 0, entry: 0, exit: 0, flash: _fees.flash}), _minLiquidity);
  }
