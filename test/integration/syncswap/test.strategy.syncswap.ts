...
describe("test.strategy.syncswapProtocol", function () {
  ...
  let i = 0;
  for (const inputSymbol of inputSymbols) {
    describe(`Test ${++i}: Syncswap ${inputSymbol}`, function () {
      this.beforeAll("Deploy and setup strat", async function () {
        ...
        const name = `Astrolab Syncswap ${inputSymbol}`;
        ...
        strategy = await setupStrat(
          ...
          name,
          {
            ...
            router: a.syncswap[inputSymbol].router,
            pool: a.syncswap[inputSymbol].pool,
