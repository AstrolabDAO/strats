  ...
  before("Deploy and setup strat", async () => {
    ...
    env = await setupStrat(
      ...
      desc.symbol, // create3 identifier
      {
        ...
        lpTokens: desc.inputs.map((i) => protocolAddr.v1.Pool[i]), // lp tokens
        rewardTokens: [addr.tokens.STG],
        extension: abiEncode( // strategy specific params
          ["address", "uint16[]"],
          [
            protocolAddr.v1.LPStaking ?? protocolAddr.v1.LPStakingTime, // staking contract
