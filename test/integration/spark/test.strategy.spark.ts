...
describe("test.strategy.sparkProtocol", function () {
  ...
  for (const inputSymbol of inputSymbols) {
    describe(`Test ${++i}: Spark ${inputSymbol}`, function () {
      this.beforeAll("Deploy and setup strat", async function () {
        ...
        const name = `Astrolab Spark sp${inputSymbol}`;
        ...
        strategy = await setupStrat(
          ...
          name,
          {
            ...
            iouToken: a.spark[inputSymbol].iouToken,
            pool: a.spark[inputSymbol].pool,
          } as ISparkStrategyV5,
          underlying,
          [underlying.address],
          [100],
